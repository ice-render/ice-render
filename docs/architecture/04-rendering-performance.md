...
0. **每组件常驻数组要当成内存成本来审**（2026-09-21，堆快照驱动的一轮优化；**这一条以及下面 0b/0c、还有
   "虚拟子源"整块内存工作的决策记录，已收敛到 [23 内存与虚拟化](23-memory-and-virtualization.md)** ——
   那里有每一刀的 A/B 数字与**否决项**，这里只留渲染性能口径）：10 万图元下实测每个图元
   ...
   **同一天的第二刀是"同几何图元共享 `Path2D`"**（见下条 0b）。
   同一天的第三~五刀见下条 0c（几何签名哈希化 / 默认监听类级声明 / 上屏快照盒 arena），
   三刀合计 **234.6 → 171.3 MB（−63.3 MB，−27.0%）**，堆对象 681.5 万 → 454.9 万。
   仍可继续挖的（见 2026-09-21 的堆分析，都带 API/序列化代价，暂不做）：
   `state.linearMatrix` 的每组件 `Float32Array(6)` + `ArrayBuffer`（10.7 MB，改成共享 slab 只能省下
   其中的 ArrayBuffer 约 5 MB，且要给它配"销毁即归还"的生命周期）、
   每组件 5 个数值数组（`composedMatrix`/`localOrigin`/`absoluteOrigin`/`absoluteLinearMatrix`/`dots`，
   约 15 MB，但矩阵按序列化契约必须是普通 `Array`）、
   每组件一个 `ICE_` + uuid 字符串（约 7.1 MB，`id` 是公开字段，改惰性生成会牵动序列化与业务绑定）、
   `__activeCtm`/`__activeWorldMatrix`（13.8 MB，依赖"按队列渲染"这一事实才能共享）。
...
0c. **第三~五刀：几何签名哈希化 + 默认监听类级声明 + 上屏快照盒 arena**（2026-09-21）：

   - **几何签名 → 双 32 位哈希**：`__pathSig` 原本是每图元一个 12~20 元素数组（实测 **11.7 MB**），
     而它只用来做"和上一次一样吗"。改成「长度 + 两个 32 位哈希」（每图元 12 字节），
     口径与逐项 `!==` 逐条对齐（`-0`≡`0`、`NaN`/`symbol` 一律判过期、对象按**引用身份**发号），
     漏判重建的概率 2⁻⁶⁴ 量级。回归：`tests/renderer/path-signature-hash.test.ts`。
   - **默认事件监听改类级声明**：见 [05 事件系统](05-event-system.md) 的"默认处理器：类级声明 + 派发时解析"。
     每图元少 3 个数组 + 3 条记录 + 1 份属性存储（实测 `Array` 130.0 万 → 90.0 万、`Object` 120.1 万 → 70.0 万）。
   - **上屏快照盒 arena**：`__snap` 原本每组件一个 `Float64Array(4)`（+ 它的 `ArrayBuffer`）——
     10 万图元实测 **10 万个 JSTypedArray + 10 万个 ArrayBuffer = 10.7 MB**，只为存 32 字节。
     现在 `WeakMap` 里存**槽位号**（Smi），数据在**分块 `Float64Array`**（4096 组件/块，128 KB/块）里；
     隐藏组件会**归还槽位**复用，结构变化时槽位从 0 重新发号（不会无限增长）。
     `getWorldBox()` 仍返回指向当前值的 `Float64Array` 视图，对外语义不变。
     回归：`tests/renderer/snapshot-arena.test.ts`。

   实测（真机 Chrome + CDP，`bench-scene.html` 10 万图元，V8 堆快照"全堆自有大小"）：
   **234.6 MB → 171.3 MB（−63.3 MB，−27.0%）**，堆节点 **681.5 万 → 454.9 万**；
   `(object elements)` 103.9 → 72.9 MB、`Object` 31.3 → 16.8 MB、`Array` 19.8 → 13.7 MB、
   `Float64Array` 5.7 MB + 相关 `ArrayBuffer` 5.0 MB → 0（只剩 arena 的 27 块，3.1 MB）。
   端到端（每场景 3 秒真实 rAF）：拖动 **53.8 → 95.8~108.7 fps**、平移 **77.7 → 105~112 fps**、
...
> **每图元成本剖面 + LOD（2026-09-21 探针结论）**：10 万图元整屏重绘 214.8ms ≈ **2.15µs/图元**，
> 其中真落墨（`ctx.fill/stroke`）**47.6%**、引擎 JS 约 **43%**、帧固定开销 ~9%（整组件跳过只剩 19.9ms）。
> 成本**均匀摊在每个图元上**，与"它是否被遮挡"无关 → 由此两条结论：
> ① **遮挡剔除不做**：平铺型场景里"被盒覆盖"的占 38.9% 数量但只 2.0% 面积，真跳过只省 **1.6%**；
> 按"逐像素安全"口径（不透明实心矩形 + 内缩圆角/描边余量）三个真实场景**一个可剔的都没有**。
> ② **LOD 落地**：`renderer.setLodMinDeviceArea(px²)`（默认 0 = 关闭）。10 万图元缩到 **0.1×** 时，
> 66.6% 的图元落到 4px² 以下，跳过它们 **237.5ms → 107.3ms（−54.8%）**；1× / 0.25× 没有亚像素图元、
> 收益为 0，所以默认关闭对既有场景零影响。命中检测不受影响（走世界盒）。
...
> **2026-09-21 三条放宽**（10 万图元压测驱动，实测见下）：
> ① 层数从 1 段放宽到 **2 段**（`MAX_LAYER_RUNS`）—— 只挑「最长的一段」时，被拖动的组件落在 z 序中部会把
> 干净区切成前后两半，另外一半每帧逐组件重画（10 万图元 **99.9ms/帧**）；取「前段 + 后段」后 **16.6ms**。
> 多段不破坏 z 序：合成按队列顺序遍历，每段在自己的 `start` 位置贴回。
> ② **纯平移手势复用层位图**（缩放不变 + 设备像素位移为整数 → 整体平移贴回，1:1 零重采样）：
> 平移期间 10 万图元 **223ms/帧 → 16.6ms**；缩放仍按原纪律回退全量。
> ③ 局部重绘的"风险相交门控"改成**先空间预筛再判类别**：`isOpaqueDrawing()` 的两条颜色正则此前
> 逐组件每帧都跑，10 万图元下占 `__collect()` 的 12.1ms（总 16.2ms）→ 预筛后 `__collect()` **7.6ms/帧**。
> 压测（真机 Chrome，10 万图元，每场景 5s）：静止 60.4 / 拖动 56.6 / 平移 57.4 / 全量动画 4.1 fps。
