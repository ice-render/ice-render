...
| 可见的 dot-path（折线/星形等）、ICEText 或非不透明落墨（alpha 色/阴影/globalAlpha/合成模式）**且与本次脏区相交** | 全量 | clip 边界与半透明落墨/字形/折线抗锯齿相交会产生接缝。**刚变脏的同类组件一律回退**：实测「文本内容变更」时字形+描边的墨迹会超出几何盒，clip 下重绘无法与全量逐像素一致（像素回归曾报 594px 差异，差异色正是文本的描边色） |
...
**仍待解决**：折线描边的 clip-AA 一致性（解决后才能让含连线的富场景也走局部重绘），与上面第三行是同一件事。
