...
| **P1** | **干净且不可缓存的「连线」会吃掉整帧的局部重绘收益** | 2026-09-11 应用层实测钉出的**真实瓶颈**：`ice-entity-designer` 里拖动实体 22/22 帧全部回退全量，阻塞源是「干净、不可缓存的 `ICEPolyLine`（关系连线）与脏区相交」—— 连线横跨画布，任何脏区都与它相交，而 `ObjectCache` 有意排除 `isLine`。解开需要二选一（**产品级取舍**）：① 缓存连线位图（clip 安全，代价是可能很大的位图）；② 接受 clip 边缘的 AA 接缝（放弃「full 与 partial 逐像素一致」这条不变量）。**在做出取舍前，局部重绘对该应用零收益** |
