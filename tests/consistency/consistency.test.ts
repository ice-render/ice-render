...
describe('findComponent 查找范围（记录当前行为与已知限制）', () => {
   ...
   * 真正生效，但实测会激活「连线端点推导（宿主事件驱动）vs 局部重绘」的既有像素不稳定——
   * `e2e/visual/dirty-rect-pixel.spec.ts` 富场景 step1 会报约 900 px 差异
   * （A=full 画布为抗锯齿混合色，B=dirty-rect 画布为饱和纯色，位于被拖动图元底边 3px 带）。
   *
   * 正确修复顺序：① 先把连线端点改为**渲染期自推导**（不依赖宿主 `AFTER_MOVE/AFTER_RENDER` 事件）；
   * ② 再放开递归查找。本用例锁定当前行为，修复后应改为断言递归命中。
   */
