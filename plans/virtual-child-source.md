...
**第 3 条「文档补丁入口」已落地**：`applyPatch?(i, patch)`（应用实现，文档唯一写入口）+
`onChildPatched?(i, patch)`（引擎在物化组件被改动后回流）+ `applyVirtualPatch(container, i, patch)`
（先写文档、再同步物化组件；同步期间不回流）。IED 实测：拖完 30 步，文档列存**立刻**等于组件位置、
`version` 从 1 递增到 91（undo/redo 因此可以记"文档补丁"而不是"组件 state 变化"）。

**第 4 条「窗口同步助手」已落地**：`syncVirtualWindow(container, { needs, map, pad, budget })`。
实现中真机抓到两个坑（都写进了文档与回归）：① `map`（访问的下标 ≠ 要物化的下标 —— IED 是
"扫符号、物化它的标注"，不写就是 `created 104 / released 104 / live 0`）；
② **回收只管自己物化的那些**（命中路径物化的条目曾被一刀切回收：点中的符号当场被拆，属性面板找不到节点）。

**第 5 条「自检与口径」已落地**：`diagnoseVirtualSource(source)` 报出"跨度超过文档跨度 x%"的盒子
（正是索引爆炸那种，且从不报异常）；虚拟容器进 worker 镜像树时**告警一次**（镜像只含物化子项，
文档本体不下发）；a11y 同口径写进 AGENTS。
