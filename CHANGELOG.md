...
- **连接插槽改为「就近吸附」：只显示、也只认离端点手柄最近的那一个**（2026-09-13）：
  旧实现把命中宿主组件上的 5 个插槽（T/R/B/L/C）**全部显示**，松手时再从 5 个里挑"第一个与手柄盒相交的"——
  用户看到的插槽未必是实际连上的那一个。现在：拖拽中只显示最近的一个（贴近到吸附距离内标黄、否则标绿），
  松手只认它（不在吸附距离内就按未命中处理）。吸附判定同时改为**强制刷新包围盒** ——
  旧实现读到的是上一帧的位置，吸附状态会比画面晚一拍。

  > 顺带澄清一个性能判断（2026-09-13 实测，ice-entity-designer 的 entity-editor.html，拖 40 步）：
  > **插槽体系本来就只有 5 个共享实例、同一时刻只归属一个宿主组件**（拖动期间 host 峰值为 1），
  > 并不存在"所有对象上的所有插槽都显示出来"。帧层面也测了：37/40 帧走局部重绘、仅 3 帧全量，
  > p50 1.1ms / p90 1.5ms（插槽可见的那些帧 p50 1.4ms / p90 2.5ms）。
  > 本次改造把"显示中的插槽"从平均 2.3 个（峰值 5）降到平均 0.4 个（峰值 1）：
  > 收益主要在**语义清晰**（用户看到的插槽 = 实际连上的插槽）与少画几个圆，而不是救帧率。
