...
> 报错，而是"命令流 / SVG 导出 / 单测全正常，屏幕上一个轮廓都没有" —— 根因都出在
> **「组件自己」与「一棵子树」的边界**上。

### 新增

- **`renderSubtreeTo(component, ctx, baseMatrix)` —— 把一棵子树渲染到指定上下文**。
  背景：`ICEComponent.renderTo()` 只画「组件自己」（子组件由渲染队列逐组件绘制，`ObjectCache` /
  静态层都依赖这一点），应用对**复合组件**（`ICEGroup` 子类）只调一次 `renderTo()` 得到的是
  **空白位图**。IED 的虚拟文档批量精灵正是这么写的：21 种工艺符号各烤一张离屏位图，结果 21 张全空，
  画面上"只有管线、一个图元都没有"（2026-09-21 真机复现）。
  新函数按**与渲染队列 / SVG 导出同源**的绘制次序递归（`paintOrderChildrenOf`：先父后子、
  同级派生件在前、各自按 `zIndex` 升序），因此离屏结果与上屏逐像素一致；
  `ICEComponent.renderTo()` 的注释同时补了醒目的"只画自己"提醒并指向它。

### 修复

- **`new Path2DRecorder()`（不传参数）默认按平台造一份原生 `Path2D`**，不再静默退化为"永不上屏的纯记录器"。
  构造参数式的历史设计下，应用 clone 构造函数（`const PathCtor = this.path2D.constructor; new PathCtor()`）
  是很自然的写法，却会拿到 `native === null` 的记录器；而引擎 3.0.0 起已不再把命令重放到 ctx，
  于是这类自定义图元变成"命令流 / 导出 / 单测全对，屏幕空白"（IED 的池/泳道、事件圆、网关菱形
  集体消失就是这一条）。**显式传 `null`** 仍保留纯记录器语义（headless / 测试桩）；
  没有原生 `Path2D` 的运行时行为完全不变。平台工厂由 `root` 用 `setPath2DNativeFactory()` 注入
  （避免 recorder ↔ root 循环依赖）。

### 回归

- `tests/renderer/render-subtree.test.ts`（4 条：容器只画自己 / 子树递归 / 次序同源 / 基准矩阵一致）；
- `tests/cross-platform/path2d-recorder.test.ts` 增补"默认造原生、显式 `null` 仍是纯记录器"。

## [4.2.0] - 2026-09-21

> **主题：内存与虚拟化。** 10 万图元的内存从 648.5 MB 压到 **171.3 MB**（累计 −73.6%），
> 官方 `setViewport` 平移 **9.9 → 116.6 fps**（长任务 28 → 0）；并新增**虚拟子源**这一档能力：
> 让看不见的图元根本不存在 —— IED 的真实水务图 **235.3 MB → 7.4 MB（−96.9%）**，100 万图元 **32 MB / 121 fps**。
> 决策记录（每一刀的 A/B 数字与**否决项**、口径与复现）见 `docs/architecture/23-memory-and-virtualization.md`。

### 新功能（虚拟子源 P2 第 3~5 条：文档补丁入口 / 窗口同步助手 / 自检）

- **文档补丁入口**（"文档是唯一真相"落到代码上）：应用实现 `applyPatch?(i, patch)` 作为**唯一写入口**；
  程序化写走 `applyVirtualPatch(container, i, patch)`（先写文档、再同步物化组件，同步期间不回流）；
  用户拖动物化组件时，引擎在物化时包的那层 `setState` 会调 `onChildPatched?(i, patch)` 回流 ——
  应用据此**立刻**更新文档并 `version++`（不必等存盘；undo/redo 因此可以记"文档补丁"）。
  IED 实测：拖 30 步后文档列存等于组件位置、`version` 1 → 91。
- **窗口同步助手** `syncVirtualWindow(container, { needs, map, pad, budget })`：省掉"每个应用各写一遍"
  的物化/回收循环，并把三个容易写错的点收进引擎：滞后带 `pad`、每帧新建上限 `budget`、
  以及 `map`（访问的下标 ≠ 要物化的下标 —— 如 IED 的"扫符号、物化它的标注"）。
  ⚠️ **回收只管自己物化的那些**：命中路径物化的条目不在管辖范围（否则用户点中的那个会被当场拆掉）。
  实现中真机抓到过这两个坑（`created 104 / released 104 / live 0`、点中的符号被回收），已进回归。
- **自检与口径**：`diagnoseVirtualSource(source)` 报出"跨度超过文档跨度 x%"的盒子（索引爆炸的根因，
  而且从不抛异常，只表现为帧率/内存莫名恶化）；虚拟容器进 worker 镜像树时**告警一次**
  （镜像只含物化子项，文档本体不下发 —— P2 之后才可能支持列存下发）；a11y 同口径写进 AGENTS。

回归：`tests/graphic/virtual-patch-sync.test.ts`（9 条）；IED `e2e/water-large.spec.ts` 增两条断言
（拖完文档立刻同步 / 跨度自检通过）。
...
（`flattenTree` + 排序）、**清空上屏快照**、丢静态层，下一帧全量 prime。而视口是**视图**状态：

- 队列成员与次序没变；
- 上屏快照存的是**世界坐标盒**，平移/缩放都不让它失效；
- 静态层自带 `(rs, ox, oy)` 栅格对齐校验，纯平移直接整体贴回（见前一批的"平移复用层位图"）。

被清掉之后的代价是复合的：每帧从零重建，**而且没有快照就没有视口裁剪**（10 万图元里屏外的那些
也要逐个走一遍渲染）。真机 Chrome + CDP 实测（100,000 图元、每帧平移 3px、3 秒真实 rAF）：

| | 改前 | 改后 |
|---|---|---|
| 官方 API `setViewport(scale, tx, ty)` 平移 | **9.9 fps**（p50 103.4ms，28 个长任务 / 2,895ms） | **116.6 fps**（p50 8.4ms，0 长任务） |
| 直改 `ice.viewport.tx`（旧写法绕开这条路） | 119.9 fps | 115.6 fps（不变） |

改法：`setViewport()` 换成 `renderer.markViewportChanged()` —— 只请求**本帧整屏重画**
（视口一变屏幕上每处落墨都错位，局部重绘会在裁剪区外留旧视图的像素），**不动**队列、快照与静态层；
视口值没变时（钳制边界 / 视口跟随的重复下发）连标记都不置。缩放变化仍然整屏重画，静态层按既有纪律
自己判定"重建 / 退回全量"（位图栅格与当时的视口绑定）。

回归：`tests/renderer/viewport-repaint.test.ts`（新增 4 条：不清队列快照、平移帧整屏重画、
缩放帧整屏重画、同值不折腾）+ 两条既有用例按新契约修正（`CanvasRenderer.culling` 的"视口变化后
当帧就按新可见区裁剪"、`static-layer` 的"只重画可见的那些"）；`npm run test:visual` 111 条全过
（含 `viewport.spec.ts`、`dirty-rect-pixel`、`static-layer-pixel`、`hidpi`、`layered` 的逐像素对拍）。
...
- **每图元成本剖面**（10 万图元、整屏重绘 214.8ms ≈ **2.15µs/图元**）：真落墨（`ctx.fill/stroke`）**47.6%**、
  引擎 JS 约 **43%**（其中 `__resetLeakyCtxState` 2.8%、style 应用 0.7%、`setTransform` 在噪声内）、
  帧固定开销 ~9%（整组件跳过只剩 **19.9ms**）。也就是说**成本均匀摊在每个图元上，与"它是否被遮挡"无关**。
- **LOD（本批落地）**：`renderer.setLodMinDeviceArea(px²)`（默认 `0` = 关闭）。设备像素面积小于阈值的图元
  不画 —— 可缩放的大画布缩到低倍率时，大量图元只剩一两个设备像素，却仍要付完整的每组件成本。
  实测（10 万图元，阈值 4px²）：**0.1× 缩略视图 237.5ms → 107.3ms（−54.8%**，跳过 66,642 个 = 66.6%）；
  1× / 0.25× 没有亚像素图元（跳过 0 个），因此**默认关闭对既有场景零影响**。
  口径：`state.width × state.height × rs²`（几何面积，不含 paint pad —— pad 会把 2×2 的图形撑成 10×10，
  拿它当可见面积会让 LOD 形同虚设）；拿不到正尺寸的组件（连线）不跳；命中检测不受影响（走世界盒）。
- **遮挡剔除：实测后决定不做**。三个真实场景（10 万图元压测场景 / IED 工艺图 / ice-game XP 桌面）：
  平铺型场景里"被别的图元盒覆盖"的占 **38.9% 数量 / 2.0% 面积**，**真跳过它们只省 1.6%**；
  按"逐像素安全"口径（覆盖者必须是不透明实心矩形、且要内缩圆角/描边余量）三个场景**一个可剔的都没有**
  （XP 的窗口是圆角 + 阴影的复合组件，不满足"完全覆盖"）。结论：2D 画家算法下"被挡在后面"≠"不用画"，
  可省的只有"被**完全不透明**块**完全覆盖**"的那部分，而它在真实内容里要么不存在、要么只值个位数百分比。
  真正的杠杆是**图元总数**（虚拟化 / 聚合 / 按需物化），不是遮挡。

### 性能（10 万图元交互：拖动 8.8 → 56.6fps、平移 4.6 → 57.4fps）

压测口径：真机 Chrome + CDP，`examples/performance/bench-scene.html` 10 万图元，每场景 5 秒真实 rAF。

- **静态层从「最长的一段连续干净组件」放宽到最多两段**（`MAX_LAYER_RUNS = 2`）。位图只能整段贴回，
  此前只挑最长段：被拖动的组件落在 z 序中部时，干净区被切成前后两半，最长的一半之外还有一半
  每帧逐组件重画 —— 实测 **99.9ms/帧（10fps）**；取「前段 + 后段」之后 **16.6ms（60fps）**。
  多段是 z 序安全的：合成时按队列顺序遍历，每段在自己的 `start` 位置贴回。
- **纯平移手势复用层位图**：缩放不变、设备像素位移是整数时，把已有层整体平移贴回（1:1、零重采样）；
  缩放仍然按原纪律回退全量。此前「视口变化帧一律不建层」让 10 万图元的平移走全量重绘
  **223ms/帧（4.6fps）**，现在 **16.6ms（57.4fps）**。
- **`__riskyIntersectsRegions` 改为「先空间预筛、再判风险类别」**：`isOpaqueDrawing()` 的两条颜色正则
  此前对**每个**组件每帧都跑，10 万图元下占 `__collect()` 的 12.1ms（总 16.2ms）；先用快照盒做 4 次比较
  筛掉绝大多数之后，`__collect()` 降到 **7.6ms/帧**。语义逐条对齐：脏组件的盒已并进脏区 → 跳过预筛；
  干净且有快照 + 与脏区不相交 → 放行；干净但无快照 → 保持保守回退。

**压测结果**（10 万图元，每场景 5s）：静止 **60.4fps** / 拖动 **56.6fps**（p50 16.6ms）/ 平移 **57.4fps**（p50 16.6ms）/
全量动画 **4.1fps**；命中检测 p50 6.5ms；内存零漂移（canvas / 2D 上下文 / `Path2D` 数量不变，堆 ±1MB）。
全量动画这一档的瓶颈是「每帧重新光栅化 10 万个图元」本身（约 260ms/帧），与缓存 / 静态层无关。
...
- **渲染期间的临时缓冲不再挂到实例上**：`__localBoxScratch`（包围盒）、`__transScratch`（平移矩阵）、
  `__absScratchB`（绝对线性矩阵的 ping-pong）、`__pathSigScratch`（几何签名采样缓冲）改为模块级共享。
  依据：这些缓冲只在**一次调用内**存活，而渲染器是**按队列逐个**调用 `doRender()`（不是父组件递归渲染
  子组件），因此跨组件共享安全。写完要落进 `state.*` 的那些（`absoluteLinearMatrix` 等）仍保留实例字段 ——
  它们是"这个组件自己的矩阵"，共享会让所有组件指向同一块内存（`__absScratchA` 就是这种）。
- **`suspendedEventNames` 按需创建**（默认 `null`）：绝大多数组件一生没挂起过事件，
  "每实例一个空数组"在 10 万图元下就是 10 万个数组。
- 实测（真机 Chrome + V8 堆快照，`examples/performance/bench-scene.html` 10 万图元，同页面同参数）：
  快照自有大小 **371.0 MB → 330.6 MB（−40.4 MB，−11%）**；节点 **11.28M → 9.58M**、
  边 **43.27M → 37.71M**、数组 **2.50M → 2.10M**；每个图元自己的数组 **38.3 → 27.3 个**。
  性能无回退（同机交错 A/B：每帧全量重绘 232/248ms → 226/240ms，拖动与静态在噪声内），
  门禁 `verify:full`（含 111 条视觉 e2e 与五套基准）与家族 12 个成员单测 + e2e 全绿。

### 修复（"不描边"被误判成半透明 → 每个图元白建一块离屏画布 + 缓存无上限）

- **`strokeStyle: 'transparent'`（= 不描边）不再被当成"半透明落墨"**。旧口径是"颜色字符串里出现
  `transparent` / `rgba(` / 8 位 hex 就算半透明"，没有看 alpha 的值 —— 而 alpha=0 的通道**根本不落墨**。
  两个后果：① dirty-rect 把这类图元当 risky 组件，富场景稳定回退全量；② `ObjectCache` 走"半透明 path"
  分支，**给每个这样的图元单独建一块离屏画布**。真机 Chrome + V8 堆快照实测
  （`examples/performance/bench-scene.html`，10 万图元）：修复前 **66,642 个图元**进了缓存 → 同样多的
  canvas + 2D 上下文；修复后**归零**（对象计数：`HTMLCanvasElement` 66,644 → 2、
  `CanvasRenderingContext2D` 66,644 → 2），同场景"每帧全量重绘"单帧 p50 **274.7ms → 254ms（−8%）**。
  家族代码里有 14 处这么写（组件库的 `ICECascader` / `ICETimePicker` / `ICETransfer`、游戏厅等）。
  现在按 **alpha 值**判定：`transparent` / `rgba(…,0)` / `#RRGGBB00` / `hsla(…,0)` 视为"该通道不落墨"；
  解析不出 alpha 的写法（如 `rgba(0,0,0)` 少参）仍保守当半透明。
- **离屏缓存的总预算对**所有**分支生效**（此前只有连线分支过）：文本 / 点集路径 / 半透明 path 三条分支
  没有上界，实测一个 10 万图元场景里半透明分支记到 **124MB**（预算本是 `MAX_CACHE_BYTES = 32MB`），
  而每个条目还额外带 canvas + 2D 上下文 ≈3KB 的固定开销。现在新增条目统一按"设备像素面积 × 4"估值过门，
  **只约束新增、已缓存的继续复用**（不做 LRU，理由与连线同：门控依赖"已缓存"这个稳定事实）。
