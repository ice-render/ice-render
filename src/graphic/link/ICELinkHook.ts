...
export default class ICELinkHook extends ICECircle {
  ...
  protected mosueDownHandler(evt: ICEEvent) {
    this.evtBus.trigger(ICE_EVENT_NAME_CONSTS.HOOK_MOUSEDOWN, new ICEEvent({ target: this }));
  }
  ...
  protected mosueMoveHandler(evt: ICEEvent) {
    this.evtBus.trigger(ICE_EVENT_NAME_CONSTS.HOOK_MOUSEMOVE, new ICEEvent({ target: this }));
  }
  /**
   *
  ...
  protected mosueUpHandler(evt: ICEEvent) {
    this.evtBus.trigger(ICE_EVENT_NAME_CONSTS.HOOK_MOUSEUP, new ICEEvent({ target: this }));
  }
