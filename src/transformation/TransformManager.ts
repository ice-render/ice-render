...
class TransformManager {
  private ice: ICE;
  private transformControl: TransformPanel;

  constructor(ice: ICE) {
    this.ice = ice;

    this.transformControl = new TransformPanel({
      left: 400,
      top: 100,
      width: 100,
      height: 100,
  ...
  private selectEvtHandler(evt) {
    //NOTE: 如果 evt.target 是 TransformPanel 自身，或者它内部的子组件，这里什么都不做，因为变换工具不能自己给自己做变换操作。
    if (evt.target && (evt.target instanceof TransformPanel || evt.target.parentNode instanceof TransformPanel)) {
      return;
    }

    //FIXME:支持多个组件的组合变换。
    //FIXME:当选中列表中只有 TransformPanel 自己，或者只有它的子组件时，隐藏变换工具。
