...
⑩ **引擎默认处理器是"类级声明 + 派发时解析"，不在每个组件上注册**（2026-09-21，内存向）：`ICEComponent.initEvents()` 只调 `__enableDefaultEvents()`，声明表在 `defaultEventListenerMap()`（`mousedown`→`mouseDownEvtHandler`、`keydown`/`keyup`→`keyboardEvtHandler`）；`trigger()` 在该事件没有实例级监听时现场解析并**先于**用户监听调用。改造前每个图元常驻 3 个数组 + 3 条记录（真机 V8 堆快照里 `Array` / `Object` 两块的大头）。纪律：① 启用开关挂在 `listeners` 对象的 **Symbol 属性**上，**不许为此给组件类加实例字段**（见"热路径类不加实例字段铁律"）；② 子类覆盖 `initEvents()` 且不调 `super` = 默认处理器不启用（`LineControlPanel` 依赖这条）；③ `off(name, 处理器, 组件)` / `off(name)` / `removeEventListener` / `hasListener` 必须继续按"它真的在 `listeners` 里"匹配；④ `purgeEvents()` 清掉开关（与改造前一起被清掉同义）；⑤ `__enableDefaultEvents()` 里那句 `markEventNameListened()` 不能漏（按需派发的前置条件）。回归：`tests/event/default-listeners.test.ts`（13 条）。
...
- **出图通道铁律（2026-09-21 确立，v4.3.0）**：`ICEComponent.renderTo(container, ctx, base)` **只画「组件自己」**—— 子组件由渲染队列**逐组件**绘制（`ObjectCache` / 静态层都依赖这条，**别改成递归**）。应用要把**一棵子树**画进离屏画布（批量精灵 / 导出缩略图 / 服务端出图）时**必须**用 `renderSubtreeTo(component, ctx, base)`（`src/renderer/render-subtree.ts`，按 `paintOrderChildrenOf` 与渲染队列 / SVG 导出同源的次序递归，离屏与上屏逐像素一致）。对复合组件只调一次 `renderTo()` 会得到**空白位图且不报错**：`ice-entity-designer` 的虚拟文档批量精灵就这样产出过 21 张全空位图，画面上"只有管线、一个图元都没有"（2026-09-21 真机复现）。配套：`Path2DRecorder` 的原生 `Path2D` 是构造参数式的历史设计，**默认值已改成"按平台自己造一份"**（`root` 用 `setPath2DNativeFactory` 注入），只有**显式传 `null`** 才是纯记录器（headless / 测试桩）—— 自定义形状 clone 构造函数（`new this.path2D.constructor()`）不会再静默拿到不上屏的记录器（IED 的池/泳道与批量精灵各踩过一次）。回归：`tests/renderer/render-subtree.test.ts`、`tests/cross-platform/path2d-recorder.test.ts`。
...
- **视口缩放铁律（2026-09-10 确立，路线 B；2026-09-21 修正视口变化的失效面）**：画布缩放是「视图缩放」而非「图元缩放」，通过 `ICE.viewport = { scale, tx, ty }` 实现（屏幕 = 世界 * scale + translate），默认单位视口不影响既有行为。约束：① `setViewport()` 只改视口状态并请求**本帧整屏重画**（`renderer.markViewportChanged()`），**严禁修改任何组件的 state**；⚠️ **不许再调 `markQueueDirty()`**（2026-09-21 修正）：视口是视图状态，队列成员没变、上屏快照存的是**世界坐标盒**（与视口无关）、静态层自带 `(rs, ox, oy)` 栅格对齐校验 —— 用结构变更的路径清掉它们等于每帧从零重建（还要丢视口裁剪）。真机 10 万图元每帧平移 3px 实测：清队列 **9.9fps / p50 103ms / 3s 内 28 个长任务**（且"视口值没变"时**一次都不要动**：重复下发同值是钳制边界/视口跟随的常态）；**严禁修改任何组件的 state**；② 组件 `render()` 在主 ctx 应用**渲染视口**（CTM = `getRenderViewport() · composedMatrix`；渲染视口 = 视口 × dpr），`renderTo()`（离屏缓存）把渲染视口编码进**自己的 base 矩阵**；③ 命中检测 `DOMEventDispatcher` 必须先把屏幕坐标 `screenToWorld()` 回世界坐标再 `containsPoint`；④ dirty-rect 的脏区在世界坐标里收集、`clearRect`/`clip` 在**渲染坐标**里，换算只有 `mapBoxToRender()` 一处（视口缩放/平移与 `dpr>1` 都走这条路，**不再回退全量**）；⑤ 离屏缓存的贴图是**整数设备像素落点 + 1:1**（详见「组件级离屏缓存铁律」的保真契约）；⑥ **视口变化帧必须整屏重画**（跳过局部重绘：视口一变屏幕上每处落墨都错位，裁剪区外会留旧视图的像素），但**照常按新可见区裁剪**（快照是世界盒，平移/缩放都不让它失效）。回归用例见 `tests/ICE.viewport.test.ts`、`tests/renderer/viewport-repaint.test.ts`、`tests/renderer/CanvasRenderer.culling.test.ts`、`e2e/visual/viewport.spec.ts`、`examples/viewport/viewport-zoom.html`。
...
- **每图元一个对象要有代价意识（2026-09-21 堆快照驱动）**：10 万图元下，一个"每个组件一个"的对象
  （哪怕是空的 `{}` 或 32 字节的 `Float64Array(4)`）在真机 V8 堆里就是 **5~10MB 量级**。已经收掉的：
  `__pathSig` 数组 → 双 32 位哈希（`tests/renderer/path-signature-hash.test.ts`）、
  默认事件监听 → 类级声明（见「事件系统铁律 ⑩」）、
  `__gradCache` / `__uiStates` → 按需创建、
  上屏快照盒 → `WeakMap` 存槽位号（Smi）+ 分块 `Float64Array` arena，隐藏组件归还槽位
...
- **虚拟子源铁律（2026-09-21 落地 P0+P3，见 `plans/virtual-child-source.md`）**：容器可以挂
  ...
  ① **坐标系是容器局部**（局部 == 世界只在"容器在 (0,0) 且无变换"时成立），引擎把可见窗口经
  **逆合成矩阵**变换过去（`visibleLocalRect`，旋转/缩放下取 4 角包围盒）—— 别拿世界盒当局部盒；
  ② **`paint` 必须被 `save/restore` 包住**：容器自己的样式在 `doRender` 之前就写进了 ctx，
  批量落墨会一路改 `fillStyle`，不隔离就会用"最后一个图元的颜色"填满整块容器（真机实测：
  整屏被紫色盖住、像素对拍 99.9% 不一致）；
  ③ **虚拟容器永不入静态层**（`__layerEligible` 里显式排除）：它的内容依赖视口，烤进位图后
  平移会把**旧窗口**整体挪走；同理它也不进组件级离屏缓存；
  ④ **窗口内的物化 / 回收走 `markWindowChanged(child)`**（`ICE.addChild` / `ICEGroup.addChild` /
  `removeChild` 里的 `notifyStructureChanged` 会自动判定）：队列照重建，但**保留其他组件的上屏快照**、
  只丢"进出那个"的快照，并要求本帧整屏重画 —— 老路径（清全部快照）在 10 万图元世界里一次 **117 ms**；
  ⑤ `childSource` **不进 `props` / `state`**（构造期被剥出来，走模块级 WeakMap 侧表）：它不可序列化，
  文档是应用的数据；`materialize()` 出来的组件才进 `childNodes`、才被序列化/镜像/a11y 看见；
  ⑥ **命中即物化 + 事件重定向（P1，2026-09-21 已落地）**：`ICEGroup.containsLocalPoint()` 走
  `source.hitTest()` 并记下命中的**下标**；派发器（只对"按下/点击/右键"这类事件；移动类事件不做
  命中检测，所以 hover 不会批量物化）与 `ICE.hitTest()` 用 `resolveVirtualHit()` 把容器换成
  那个下标物化出来的**真组件** ⇒ 选中 / 控制面板 / 拖动零改动可用。三条纪律：
  ⓐ **命中 = 命中子项**，不回退到容器自己的盒子（虚拟容器的盒通常覆盖整个世界，回退会让空白处
  命中巨型容器）；ⓑ 物化必须**幂等**（`materializeVirtualChild`，引擎维护 `容器→(下标→组件)`），
  回收走 `releaseVirtualChild`，应用实现 `materialize(i)` 时**必须**在文档里标记"已物化"否则画两遍；
  ⓒ 命中策略可切换（`setVirtualHitPolicy(container, 'materialize' | 'container')`，默认前者）。
  对齐参考线（`AlignmentGuideManager`）对虚拟容器用 `forEachInBox` + `boxAt` 取候选 —— 否则
  "对齐到看不见的图元"直接失效。
  ⑦ **导出与序列化走"全量文档"，不是"窗口子集"（P2 第 1+2 条，2026-09-21 已落地）**：
  `paintToSvg?(sink, bounds)` 把**整份文档**写进 SVG sink（`define`/`use` 复用复合符号；
  引擎包 `<g transform>`、拼 `<defs>`、保证"批量内容在物化子项之前"的次序），
  `documentBounds?(out)` 供 `area:'content'` 用；序列化时容器写
  `virtual: { type, count, version, payload }`（`payload` 是**应用的数据**，引擎原样存取），
  物化子项写 `virtualIndex`；反序列化按 `type` 经 `ICE.registerVirtualSource(type, factory)`
  找工厂重建，并用 `virtualIndex` 把子项重新登进"已物化"表（**不登就会画两遍** ——
...
| `ice-trading-chart` | **8102** | 示例页冒烟（2026-09-21 补） |
